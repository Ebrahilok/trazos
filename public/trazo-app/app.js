...
function updateGlyphStatus() { const character = [...$('#glyphCharacter').value.toLowerCase()].at(-1) || 'a'; $('#glyphCharacter').value = character; const count = (currentProject().glyphs[character] || []).length; $('#glyphCount').textContent = `${count}/10`; $('#glyphProgress').style.width = `${Math.min(100, count * 10)}%`; $('#glyphMessage').textContent = count >= 10 ? 'Lista para variar con naturalidad.' : `Faltan ${10 - count} muestras recomendadas.`; }
...
function addHumanizedText(value) {
  ...
  [...value].forEach((character, index) => {
    ...
    if (variants?.length) {
      ...
      const paths = variant.map((stroke) => new fabric.Polyline(stroke.map((point) => ({ x: point.x, y: point.y })), { fill: '', stroke: $('#objectColor').value, strokeWidth: 5, strokeLineCap: 'round', strokeLineJoin: 'round', selectable: false }));
      object = new fabric.Group(paths, { left: x, top: y, scaleX: size / 115, scaleY: size / 115, selectable: false }); x += size * .72;
    } else { object = new fabric.Text(character, { left: x, top: y, fontSize: size, fill: $('#objectColor').value, fontFamily: 'Segoe Print, Comic Sans MS, cursive', selectable: false }); x += object.width + (Math.random() - .5) * 4; }
    object.angle = (Math.random() - .5) * 3 * Number($('#humanize').value) / 100; object.top += (Math.random() - .5) * 5; group.push(object);
  });
